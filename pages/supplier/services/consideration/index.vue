...
// Функция для получения данных товаров/услуг на рассмотрении
const fetchProducts = async () => {
    try {
        ...
        error.value = err
        console.error('Ошибка при загрузке продуктов на рассмотрении:', err)
    } finally {
        loading.value = false
    }
}
...
<template>
    <NuxtLayout name="supplier">
        ...
        <div v-if="loading" class="w-full bg-white rounded-[16px] mt-[20px] py-[20px] px-[12px] text-center">
            ...
            <p class="mt-4 text-gray-600">Загрузка товаров и услуг на рассмотрении...</p>
        </div>
        ...
        <div v-if="!loading && !error && products.length === 0" class="w-full bg-white rounded-[16px] mt-[20px] py-[20px] px-[12px] text-center">
            <p class="text-gray-600">У вас пока нет товаров и услуг на рассмотрении</p>
        </div>
