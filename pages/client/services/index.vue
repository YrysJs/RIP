...
async function openReceiptModal(order) {
    try {
        ...
        const transactionId = order.transaction_id || order.payment_transaction_id || order.payment_id || order.transaction || order.id
        
        if (!transactionId) {
            console.error('Transaction ID не найден для заказа:', order)
            alert('Ошибка: Transaction ID не найден. Проверьте консоль для подробностей.')
            return
        }
...
<template>
    <NuxtLayout name="client">
        <div class="w-full bg-white rounded-[16px] mt-[20px] py-[20px] px-[12px] flex flex-col gap-[30px]">
            ...
            <!-- Пагинация -->
            <div v-if="orders?.items?.length && totalPages > 1" class="flex justify-between items-center mt-8 p-4 bg-gray-50 rounded-lg">
                <div class="flex items-center gap-4">
                    <span class="text-sm text-gray-600">
                        Показано {{ (currentPage - 1) * pageSize + 1 }}-{{ Math.min(currentPage * pageSize, totalCount) }} из {{ totalCount }} заказов
                    </span>
                    <select v-model="pageSize" @change="changePageSize(pageSize)" class="px-3 py-1 border rounded text-sm">
